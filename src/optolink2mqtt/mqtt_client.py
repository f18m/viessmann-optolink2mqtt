...
class MqttClient:
    ...
    OPTOLINK2MQTT_STATUS_TOPIC = "optolink2mqtt_status"
    ...
    def __init__(
        ...
        self._mqttc.will_set(
            self.get_optolink2mqtt_status_topic(),
            payload=MqttClient.LAST_WILL_PAYLOAD,
            qos=0,
            retain=True,
        )
        return
    ...
    def connect(
        self, mqtt_broker: str, mqtt_port: int, username: str, password: Optional[str]
    ) -> bool:
        """
        ...
        logging.info(
            f"Connecting to MQTT broker '{mqtt_broker}:{mqtt_port}' with client_id={self.client_id}"
        )
        rc = self._mqttc.connect(mqtt_broker, mqtt_port)
        logging.info(
            f"Connecting to MQTT broker '{mqtt_broker}:{mqtt_port}'... return code was {rc}"
        )
    ...
    def loop_start(self) -> None:
        ...
        logging.info(
            f"started MQTT client loop for connection_id={self._connection_id}; return code was {rc}"
        )
    ...
    def loop_stop(self) -> None:
        ...
        logging.info(
            f"stopped MQTT client loop for connection_id={self._connection_id}; return code was {rc}"
        )
    ...
    def get_optolink2mqtt_status_topic(self) -> str:
        """
        Returns the topic used to publish metrics related to OPTOLINK2MQTT itself
        """
    ...
    def on_connect(
        ...
        if reason_code != 0:
            logging.warning(
                f"Connected to MQTT broker with reason_code={reason_code}, connection_id={self._connection_id}"
            )
        else:
            logging.info(
                f"Successfully connected to MQTT broker with connection_id={self._connection_id}"
            )

        # update our status:
        self._mqttc.publish(
            self.get_optolink2mqtt_status_topic(),
            MqttClient.NEW_CONN_PAYLOAD,
            qos=self.qos,
            retain=True,
        )
        ...
        if self.ha_status_topic != "":
            logging.info(
                f"Subscribing to HomeAssistant status topic '{self.ha_status_topic}'"
            )
    ...
    def on_disconnect(
        ...
        if reason_code != 0:
            ...
            logging.warning(
                f"OOOOPS! Unexpected disconnection from the MQTT broker with reason=[{reason_code}] for connection_id={self._connection_id}. Reconnecting in {self.reconnect_period_sec}sec."
            )
    ...
    def on_message(
        self, mqttc: paho.Client, userdata: Any, msg: paho.MQTTMessage
    ) -> None:
        """
        ...
        if msg.topic.startswith(self.request_topic):
            logging.error("Feature not yet implemented. TODO.")
        elif msg.topic == self.ha_status_topic:
            mqtt_payload = msg.payload.decode("UTF-8")
            if mqtt_payload == "online":
                logging.info(
                    "HomeAssistant status changed to 'online'. Need to publish MQTT discovery messages."
                )
